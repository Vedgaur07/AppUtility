...
public class StringHelper {
    ...
    private static final String STRING_INT_ACCEPT_REGEX = "[^a-zA-Z0-9]";
    private static final String INT_ACCEPT_REGEX = "[^0-9]";
    private static final String DOUBLE_ACCEPT_REGEX = "[^0-9.]";
...
//	}
    ...
    public static String replaceData(String str, String replaceBy, String... replaceTo) {
        String modifyStr = str;
        for (String s : replaceTo) {
            modifyStr = str.replace(s, replaceBy);
        }
        return modifyStr;
    }
    ...
    public static Number getNumberFromString(String val) {
    ...
    public static int toInteger(String val) {
        int num = 0;
        if (!isEmpty(val)) {
            val = val.replaceAll(INT_ACCEPT_REGEX, "");
    ...
    public static double toDouble(String val) {
        double num = 0.0;
        if (!isEmpty(val)) {
            val = val.replaceAll(DOUBLE_ACCEPT_REGEX, "");
            num = Double.parseDouble(val);
        }
        return num;
    }

    public static int stringDoubleCastInInt(String val) {
        int num = 0;
        if (!isEmpty(val)) {
            val = val.replaceAll(DOUBLE_ACCEPT_REGEX, "");
            num = (int) Math.round(Double.parseDouble(val));
        }
        return num;
    }

    public static int doubleCastInInt(double val) {
        int num = 0;
        num = (int) Math.round(val);
        return num;
    }

    public static double stringChangeToDouble(String val) {
        double num = 0.0;
        if (!isEmpty(val)) {
            val = val.replaceAll(DOUBLE_ACCEPT_REGEX, "");
            num = Double.parseDouble(val);
    ...
    public static String getFirstLetterFromString(String str) {
        String val = "";
        if (!isEmpty(str)) {
            String[] splitStr = str.split(" ");
            for (String s : splitStr) {
                val = val.concat(String.valueOf(s.charAt(0)));
            }
        }
        return val;
    }
